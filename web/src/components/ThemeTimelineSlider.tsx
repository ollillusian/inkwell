...
function dayNarrative(step: ThemeTimelineStep): string {
  ...
  if (c.firstDay) {
    return `Your first map in this span: ${step.stats.topics} themes and ${step.stats.links} links from ${step.entryCount} ${step.entryCount === 1 ? "entry" : "entries"}.`;
  }
  const bits: string[] = [];
  ...
  if (c.topicsAgain.length > 0) {
    bits.push(`kept ${listSnippet(c.topicsAgain)} from the day before`);
  }
  if (c.newPhrases.length > 0) {
    bits.push(`new phrases like “${c.newPhrases[0]}”`);
  }
  if (c.strongerTopics.length > 0 && bits.length < 2) {
    bits.push(`${c.strongerTopics[0]} showed up more`);
  }
  if (bits.length === 0) {
    return "Quiet day — the map mostly reshuffled links without new themes.";
  }
  return `${step.weekday}: ${bits.join("; ")}.`;
}
...
function StatPill({
  ...
  return (
    <div className="rounded-xl border border-ink-border/80 bg-ink-bg/60 px-3 py-2 min-w-[4.5rem]">
...
function ChangeChip({
  ...
  return (
    <li
      className={`rounded-full border px-2.5 py-1 text-[11px] ${toneClass}`}
    >
      {label}
    </li>
  );
...
export function ThemeTimelineSlider({ steps }: Props) {
  ...
  return (
    <div
      className="lg:grid lg:grid-cols-[minmax(0,22rem)_1fr] lg:gap-8 lg:items-start space-y-6"
      onTouchStart={onTouchStart}
      onTouchEnd={onTouchEnd}
    >
      <div className="space-y-4 lg:sticky lg:top-4">
        <section className="rounded-2xl border border-ink-border bg-gradient-to-b from-ink-surface to-ink-surface/40 px-4 py-4 space-y-4 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <p
                key={step.dateKey}
                className="font-serif text-2xl mt-1 timeline-day-enter"
              >
                {step.dateLabel}
              </p>
            </div>
            <Link
              ...
              className="text-sm text-ink-accent hover:underline shrink-0 pt-1"
            >
              Open day →
            </Link>
          </div>
          ...
          <p
            key={`${step.dateKey}-story`}
            className="text-sm text-ink-muted leading-relaxed border-l-2 border-ink-accent/50 pl-3 timeline-day-enter"
          >
            {dayNarrative(step)}
          </p>
          ...
          <div
            ref={filmstripRef}
            className="timeline-filmstrip flex gap-1 overflow-x-auto pb-2 -mx-1 px-1 scroll-smooth"
            role="tablist"
            aria-label="Days in timeline"
          >
            {steps.map((s, i) => {
              const active = i === index;
              const h = 20 + linkScale[i]! * 28;
              return (
                <button
                  key={s.dateKey}
                  type="button"
                  ...
                  onClick={() => go(i)}
                  className={`flex flex-col items-center justify-end shrink-0 rounded-lg px-1.5 pt-2 pb-1 transition-all min-w-[2.4rem] ${
                    active
                      ? "bg-ink-fg text-ink-bg scale-105 shadow-md"
                      : "bg-ink-bg/80 text-ink-muted hover:bg-ink-surface border border-ink-border/60"
                  }`}
                  style={{ height: `${h + 36}px` }}
                >
                  <span
                    className={`w-2 rounded-full mb-1 transition-colors ${
                  ...
                  />
                  <span
                    className={`text-[9px] leading-tight text-center max-w-[3rem] ${
                      active ? "font-medium" : ""
                    }`}
                  >
          ...
          <div className="space-y-2">
            <input
              ...
              className="w-full h-1.5 rounded-full appearance-none bg-ink-border/60 accent-ink-accent cursor-pointer"
              aria-label="Scrub through days"
            />
            <div className="flex justify-between text-[10px] text-ink-muted">
              <span>{steps[0]?.shortLabel}</span>
              <span>{steps[maxIndex]?.shortLabel}</span>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              {highlightNew ? "What's new: on" : "What's new: off"}
            </button>
            <button
              type="button"
              disabled={index <= 0}
              onClick={() => go(index - 1)}
              className="rounded-full border border-ink-border px-3 py-1.5 text-xs disabled:opacity-35"
          ...
          <p className="text-[10px] text-ink-muted">
            ← → to step · Space to play · Swipe the map on mobile
          </p>
        </section>

        {c && (
          <section className="rounded-2xl border border-ink-border bg-ink-surface/50 px-4 py-3 space-y-2">
            <h2 className="text-[10px] font-medium uppercase tracking-wide text-ink-muted">
              Threads from yesterday
            </h2>
            {c.firstDay ? (
              <p className="text-xs text-ink-muted leading-relaxed">
                Everything below is new for this window — no prior day to compare.
              </p>
            ) : (
              <ul className="flex flex-wrap gap-1.5">
                ...
                {c.newTopics.length === 0 &&
                  ...
                  c.newPhrases.length === 0 && (
                    <li className="text-xs text-ink-muted">No new threads — map shifted shape only.</li>
                  )}
              </ul>
            )}
          </section>
      ...
      <section className="space-y-3 min-w-0">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          {highlightNew && pulseIds.length > 0 && (
            <span className="text-[10px] text-ink-accent">
              Pulsing ring = new since prior day
            </span>
          )}
        </div>
        <div key={step.dateKey} className="timeline-day-enter">
          <ThemeNetworkGraph
            graph={step.graph}
