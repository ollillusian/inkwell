  ...
  return (
    <div className="min-h-full flex flex-col bg-ink-bg text-ink-fg">
      <header className="sticky top-0 z-10 border-b border-ink-border/50 bg-ink-bg/90 backdrop-blur-md">
        <div
          className="app-content-column flex items-center justify-between px-4 sm:px-5 py-4"
        >
        ...
        <nav
          className="app-content-column flex gap-1 px-4 sm:px-5 pb-3"
        >
          {nav.map((item) => {
            const active =
              item.href === "/app"
      ...
      <main
        className="app-content-column flex-1 px-4 sm:px-5 py-8"
      >
        {children}
      </main>
    </div>
