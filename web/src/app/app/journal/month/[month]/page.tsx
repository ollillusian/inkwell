...
export default async function JournalMonthPage({ params }: Props) {
  const { month: monthKey } = await params;
  if (!normalizeMonthKey(monthKey)) notFound();
  ...
  const monthEntries = entriesForLocalMonth(all, monthKey, timeZone);

  if (monthEntries.length === 0) {
    return (
      <div className="space-y-6">
        <Link href="/app/journal?view=months" className="text-sm text-ink-accent">
          ← Journal
        </Link>
        <p className="font-serif text-2xl text-ink-muted">No entries this month</p>
      </div>
    );
  }
  ...
  const totalWords = monthEntries.reduce((n, e) => n + wordCount(e.body), 0);
  const dayGroups = dayGroupsInMonth(all, monthKey, timeZone);
  const title = formatMonthLabel(monthKey, timeZone);
    ...
    <div className="space-y-10 pb-8">
      <div>
        ...
        <p className="mt-1 text-sm text-ink-muted">
          {monthEntries.length}{" "}
          {monthEntries.length === 1 ? "entry" : "entries"} · {dayGroups.length}{" "}
      ...
      <section className="space-y-3">
        ...
        <p className="text-xs text-ink-muted -mt-1">
          Each inner node is a day you wrote; themes sit on the outer ring.
        </p>
